import { generateCSS, updateCSS } from './generate-css';
    ...
    return createHigherOrderComponent((OriginalComponent) => {
        return class WrappedComponent extends Component {
            ...
            saveCSS = (responsiveCSS, nonResponsiveCSS) => {
                ...
                _CSS = _CSS.replace(new RegExp('{{WPRIG}}', "g"), '.wprig-block-' + uniqueId)

                let styleSelector = window.document;
                if (styleSelector.getElementById('wprig-block-' + uniqueId) === null) {
                    let cssInline = document.createElement('style');
                    cssInline.type = 'text/css';
                    cssInline.id = 'wprig-block-' + uniqueId;
                    ...
                    styleSelector.getElementById('wprig-block-' + uniqueId).innerHTML = _CSS;
                }
            }
                ...
                if (changedAttributes.length > 0) {
                    ...
                    if (!isLayoutChanged) {
                        if (changedAttributes.indexOf('uniqueId') !== -1) {
                            let currentStyleElement = window.document.getElementById('wprig-block-' + prevProps.attributes.uniqueId);
                            if (currentStyleElement && !isDuplicatingBlock(getBlocks())) {
                                currentStyleElement.id = 'wprig-block-' + attributes.uniqueId;
            ...
            copyAttributes = () => {
                const {
                    attributes,
                    attributes: {
                        wprigStyleAttributes
                    }
                } = this.props;
                const { copyToClipboard } = wp.wprigComponents.HelperFunction;
                let template = {}
                wprigStyleAttributes.forEach(key => {
