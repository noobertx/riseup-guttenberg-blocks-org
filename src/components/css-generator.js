...
const replaceWarp = (selector, ID) => {
    return selector.replace(new RegExp('{{WPRIG}}', "g"), '.wprig-block-' + ID)
}
...
export const CssGenerator = (settings, blockName, blockID, isInline = false, global = false, frontend = false, globalAttributes = undefined) => {
    ...
    Object.keys(settings).forEach((key) => {
        let attributes = [];
        if (global === false) {
            attributes = typeof blockName === 'string' ? wp.blocks.getBlockType('wprig/' + blockName).attributes : blockName
        } else {
    ...
    if (global) {
        __CSS = __CSS.replace(new RegExp('.wprig-block-global', "g"), frontend ? '.wprig-frontend' : '.wprig-editor');
    }
...
const setStyle = (styleCss, blockID, global = false, frontend = false) => {
    let styleSelector = window.document, id = 'wprig-block-' + blockID;
    if (global && frontend) {
        id = 'wprig-global-styles';
    }
