const PATH = '/wprig/v1/global_settings';
import { _device, _push } from '../components/css-helper';
...
export const setTypoTitleStyle = (typos) => {
    ...
    injectGlobalCSS(__CSS, 'wprig-global-panel')
}
...
const appendTypoVariable = (value, index) => {
    ...
    if (value.size) {
        // data = _appendVariables(generateVariables(value.size, `--wprig-typo${index + 1}-font-size:{{key}};`), data);
    }
    if (value.height) {
        // data = _appendVariables(generateVariables(value.height, `--wprig-typo${index + 1}-line-height:{{key}};`), data)
    }
    if (value.spacing) {
        // data = _appendVariables(generateVariables(value.spacing, `--wprig-typo${index + 1}-letter-spacing:{{key}};`), data)
    }
    ...
    if (value.family) {
        // nonResponsiveProps += `--wprig-typo${index + 1}-font-family:'${value.family}',${value.type};`;
    }
    if (value.weight) {
        if (typeof value.weight === 'string') {
            // nonResponsiveProps += `--wprig-typo${index + 1}-font-weight:${value.weight.slice(0, -1)};`;
            // nonResponsiveProps += `--wprig-typo${index + 1}-font-style:italic;`;
        } else {
            // nonResponsiveProps += `--wprig-typo${index + 1}-font-weight:${value.weight};`;
            // nonResponsiveProps += `--wprig-typo${index + 1}-font-style:normal;`;
        }
    }
    if (value.transform) {
        // nonResponsiveProps += `--wprig-typo${index + 1}-text-transform:${value.transform};`;
    }
...
const setBreakingPoints = (breakingPoints) => {
    ...
    if (typeof sm !== 'undefined') {
        tempCSS += `@media (min-width: 576px){.wprig-section .wprig-container{max-width: ${sm}px;}}`
    }
    if (typeof md !== 'undefined') {
        tempCSS += `@media (min-width: 768px){.wprig-section .wprig-container{max-width: ${md}px;}}`
    }
    if (typeof lg !== 'undefined') {
        tempCSS += `@media (min-width: 992px){.wprig-section .wprig-container{max-width: ${lg}px;}}`
    }
    if (typeof xl !== 'undefined') {
        tempCSS += `@media (min-width: 1200px){.wprig-section .wprig-container{max-width: ${xl}px;}}`
    }
    if (typeof xlg !== 'undefined') {
        tempCSS += `@media (min-width: 1200px){.wprig-section .wprig-container{max-width: ${xlg}vw;}}`
    }
    return tempCSS;
}

export const injectGlobalCSS = (_CSS, id = 'wprig-global-styles') => {
...
export const updateGlobalVaribales = async (presetValues, breakingPoints = undefined) => {
    ...
    const setGlobalCSS_Variables = (colors) => {
        let rootCSS = ':root {'
        colors.forEach((color, index) => rootCSS += `--wprig-color-${index + 1}:${color};`);
        rootCSS += '}'
        return rootCSS;
    }
...
export const getGlobalSettings = () => {
    ...
    let breakingPoints = {
        ...
        ...(typeof wprig_container_width !== undefined && wprig_container_width)
    }
    return fetchFromApi().then(data => {
        if (data.success) {
            ...
            const setGlobalCSS_Variables = (globalColors) => {
                let rootCSS = ':root {'
                globalColors.forEach((color, index) => rootCSS += `--wprig-color-${index + 1}:${color};`);
                rootCSS += '}'
                return rootCSS;
            }
