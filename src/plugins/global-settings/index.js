...
class GlobalSettings extends Component {
    ...
    async saveGlobalCSS() {
        ...
        await injectGlobalCSS(_CSS, 'wprig-global-styles');
    }
    ...
    getGlobalSettings = () => {
        ...
        return fetchFromApi().then(data => {
            if (data.success) {
                ...
                localStorage.setItem('wprig-global-settings', JSON.stringify({
    ...
    render() {
        ...
        const renderPresets = () => {
            ...
            const deletePreset = (selectedPreset) => {
                this.setState(prevState => {
                    ...
                    localStorage.setItem('wprig-global-settings', JSON.stringify(presets[selectedPreset === prevState.activePreset ? 'preset1' : activePreset]));
                    return ({
                        presets: prevState.presets,
            ...
            return (
                <div className="wprig-global-settings" >
                    {
...

                                    {
                                        ...
                                        <Fragment>
                                            ...
                                            <PanelBody title={__(presetKey === 'theme' ? 'Theme Colors' : 'Global Colors')} initialOpen={true}>
                                                <div className="wprig-global-color-pallete">
                                                    {
        ...
        const updateBreakingPoints = (key, newValue) => {
            this.setState(({ presets, activePreset, breakingPoints }) => {
                ...
                if (typeof wprig_container_width !== undefined) {
                    defaultValue = wprig_container_width[key]
                } else {
                    if (key === 'sm') {
                        defaultValue = 720;
                    } else if (key === 'md') {
        ...
        localStorage.setItem('wprig-global-settings', JSON.stringify({ ...presets[activePreset], breakingPoints }));
        return (
            <Fragment>
                <PluginSidebar
                    icon= 'universal-access-alt'
                    name="global-settings-sidebar"
                    title={__('wprig Global Settings')}
                >
                    {renderPresets()}
                    <PanelBody title={__('Global Container Width')} initialOpen={false}>
                        ...
                        />
                        <div className="wprig-row-device" style={{ margin: '0 -15px' }}>
                            <Notice status="default" isDismissible={false}>
                                <div className="wprig-device-description title">{__('Device definations in terms of ')}<strong>{__('min-width')}</strong></div>
                                <div className="wprig-device-description">{__('Landscape Mobile : 576px')}</div>
                                <div className="wprig-device-description">  {__('Tablet : 768px')}</div>
                                <div className="wprig-device-description">  {__('Desktop : 992px')}</div>
                                <div className="wprig-device-description">{__('Large Desktop : 1200px')}</div>
                            </Notice>
                        </div>
